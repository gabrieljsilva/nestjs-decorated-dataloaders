...
describe("DataloaderContext", () => {
	...
	let dataloaderService: DataloaderContext;

	beforeEach(() => {
		LazyMetadataContainer.clear();
		handlerResolver = new SimpleHandlerResolver();
		dataloaderService = new DataloaderContext({ handlerResolver });
	});
	...
	it("should load one-to-many relationships", async () => {
		...
		handlerResolver.register(PhotoRepository, PhotoRepository);

		LazyMetadataContainer.loadRelationshipMetadata();

		const users = factory.newList(User, 3);
		const results = await Promise.all(
			users.map((user) => dataloaderService.load({ from: User, field: "photos", parent: user })),
		);
	...
	it("should load one-to-one relationships", async () => {
		...
		handlerResolver.register(ProfileRepository, ProfileRepository);

		LazyMetadataContainer.loadRelationshipMetadata();

		const users = factory.newList(User, 3);
		const results = await Promise.all(
			users.map((user) => dataloaderService.load({ from: User, field: "profile", parent: user })),
		);
	...
	it("should load one-to-many relationships by intermediate entity", async () => {
		...
		handlerResolver.register(TagByProjectRepository, TagByProjectRepository);
	...
	it("should load one-to-one relationships by an intermediate entity", async () => {
		...
		handlerResolver.register(AddressRepository, AddressRepository);
	...
	it("should clear all cached entries for a specific parent and field", async () => {
		...
		handlerResolver.register(ProductRepository, ProductRepository);
	...
	it("should clear specific entries without affecting other cached entries", async () => {
		...
		handlerResolver.register(StudentRepository, StudentRepository);
	...
	it("should correctly prime the dataloader with new values", async () => {
		...
		handlerResolver.register(EmployeeRepository, EmployeeRepository);
	...
	it("should load data by many keys", async () => {
		...
		handlerResolver.register(OrderRepository, OrderRepository);
