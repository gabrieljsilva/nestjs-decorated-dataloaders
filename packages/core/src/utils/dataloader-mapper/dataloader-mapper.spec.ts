...
describe("DataloaderMapper", () => {
	...
	// Composite keys (a key made of multiple columns, e.g. tenantId + code).
	// These document a current bug: an array is overloaded to mean "a list of N keys"
	// (the many-to-many case), so there is no way to express "an array that IS one
	// composite key". The lookup also fails because Map.get uses reference equality
	// on the array key. Marked with `it.fails`: once a canonical/serialized key is
	// introduced these should pass — remove `.fails` then.
	describe("composite keys (known bug)", () => {
		const compositeKey = (entity: any) => [entity.tenantId, entity.code];

		it.fails("should map a one-to-one relationship by a composite key", () => {
			const metadata: LoadedRelationship = {
				...metadataOneToOne,
			...
			const keys: Array<JoinProperty> = [
				[1, "a"],
				[2, "b"],
			] as unknown as Array<JoinProperty>;
			const entities = [
				{ tenantId: 1, code: "a", name: "Entity1" },
				{ tenantId: 2, code: "b", name: "Entity2" },
			];
		...
		it.fails("should group a one-to-many relationship by a composite key", () => {
			const metadata: LoadedRelationship = {
				...metadataOneToMany,
				parentKey: compositeKey,
			};
			...
			const entities = [
				{ tenantId: 1, code: "a", name: "Entity1" },
				{ tenantId: 1, code: "a", name: "Entity2" },
				{ tenantId: 2, code: "b", name: "Entity3" },
			];
